...
buf.push("<a" + (jade.attrs({ 'href':('/' + (post.author) + '') }, {"href":true})) + ">");
jade.debug.unshift({ lineno: undefined, filename: jade.debug[0].filename });
jade.debug.unshift({ lineno: 16, filename: jade.debug[0].filename });
buf.push("<img" + (jade.attrs({ 'src':('' + (post.author) + ''), "class": [('icon')] }, {"src":true})) + "/>");
jade.debug.shift();
...
buf.push("" + (jade.escape((jade.interp = post.author) == null ? '' : jade.interp)) + "");
jade.debug.shift();
jade.debug.shift();
buf.push("</div>");
jade.debug.shift();
